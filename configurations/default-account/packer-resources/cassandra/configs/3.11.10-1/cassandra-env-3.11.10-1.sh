...
if [[ "$LOCAL_JMX" = "yes" ]]; then
  ...
  JVM_OPTS="$JVM_OPTS -Djavax.net.ssl.keyStore=/etc/cassandra/keystores/server-keystore.jks"
  JVM_OPTS="$JVM_OPTS -Djavax.net.ssl.keyStorePassword=##KEYSTORE_PASS##"
  JVM_OPTS="$JVM_OPTS -Djavax.net.ssl.trustStore=/etc/cassandra/keystores/server-truststore.jks"
  ...
  JVM_OPTS="$JVM_OPTS -Dcom.sun.management.jmxremote.ssl.enabled.protocols=TLSv1"
...
if [ $DEFINED_XMN -eq 0 ] && [ $DEFINED_XMX -ne 0 ]; then
    echo "Please set or unset -Xmn flags in jvm.options file."
    exit 1

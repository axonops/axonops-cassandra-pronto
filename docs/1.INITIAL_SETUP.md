...
Usage:
  create-configuration.sh
    ...
    -c : [Required] cluster name (e.g. "cassandra-cluster" or "storage-cluster")
...
configured as you intend.
  * The region should match where you plan to deploy your Cassandra resources.
...
Generate SSH keys for Ansible and collect your own key with the [init-ansible-key.sh](../core-components/scripts/ssh/init-ansible-key.sh)
script. This script must be run before baking the AMI so the Ansible public key can be baked into the image.
```
...
* The `-u` and `-n` options should point to public SSH keys (i.e. `~/.ssh/id_rsa.pub`).
  * These can also be configured as `TERRAFORM_SSH_KEY_PATH` and `TERRAFORM_ANSIBLE_KEY_PATH` in your [variables.yaml](../configurations/default-account/variables.yaml).
* If the Ansible key doesn't exist at the configured location, the script creates a new 2048-bit RSA key.
* If your personal user key doesn't exist, the script exits with an error — you must have your own SSH key first.
* The generated `user-keys.yaml` file is placed in your cluster configuration directory and is required for both AMI baking and Terraform deployments.

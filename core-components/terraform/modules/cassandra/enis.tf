resource "aws_network_interface" "cas-seed-eni" {
  ...
  tags = merge(tomap({ "Name" = "${var.cluster_name}-seed-${count.index}" }), var.ec2_tags, local.required_ec2_tags)
}
...
resource "aws_network_interface" "cas-non-seed-eni" {
  ...
  tags = merge(tomap({ "Name" = "${var.cluster_name}-non-seed-${count.index}" }), var.ec2_tags, local.required_ec2_tags)

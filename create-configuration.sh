...
# portable relative-path resolution (GNU `realpath --relative-to` substitute;
# BSD/macOS realpath has no --relative-to flag)
relpath() {
...
usage() {
  ...
  echo "    -a : [Required] account name (e.g. 'my-cassandra-account')"
  echo "    -v : [Required] vpc name (e.g. 'primary-vpc' or 'west-vpc')"
  echo "    -c : [Required] cluster name (e.g. 'cassandra-cluster' or 'storage-cluster')"
...
# validate each name independently (must start with a letter, alnum + dash only)
name_re='^[a-zA-Z][a-zA-Z0-9-]+$'
for n in "${ACCOUNT_NAME}" "${VPC_NAME}" "${CLUSTER_NAME}"; do
  if [[ ! "$n" =~ $name_re ]]; then
    echo "ERROR: Account name (-a), VPC name (-v), and Cluster name (-c) must each start with a letter, and contain only alphanumeric characters (and dashes)." >&2
    exit 1
  fi
done
...
if [[ ! -d "${NEW_VPC_DIR}" ]]; then
  ...
  if [[ ${TF_MANAGED_VPC} = false ]]; then
    ...
    echo "  - Copied from:  $(relpath "${DEFAULT_VPC_DIR}/vpc-resources/vpc-existing.tfvars" .)"
    ...
    echo "  - Copied from:  $(relpath "${DEFAULT_VPC_DIR}/vpc-resources/vpc-new.tfvars" .)"
...
echo "  - Profile is in dir:  $(relpath "${NEW_CLUSTER_DIR}" .)"
echo "  - Customize your configs as needed; fill in any variables left blank (look for \"<<< XYZ_HERE >>>\")."
echo "      $ find $(relpath "${NEW_ACCOUNT_DIR}" .) -type f | xargs grep \"<<< .* >>>\""
